...
public class JiraController {
    ...
    public Message raiseJiraTicket(@RequestBody MultipartFile data) throws IOException {
        ...
        File file = createAndWriteFile(csvContents, userToken.getUserId());
        
        return createJiraTicket(file);
    }

    private File createAndWriteFile(String csvContents, String userId) throws IOException {
        if (csvContents == null) {
            return null;
        }

        File dir = createDirectoryIfNotExists();
        String fileName = String.format(FILE_NAME_FORMAT, FileSeparator.CSV_SEPARATOR.getName(), System.currentTimeMillis(), userId);

...
public class JiraController {
  ...
  public Message raiseJiraTicket(@RequestBody MultipartFile data) throws IOException {
    ...
    // Raise TSUP ticket
    return file.map(this::raiseTSUPTicket)
               .orElseThrow(() -> new IOException("Failed to create CSV file"));
  }

  private Optional<File> createCSVFile(String csvContents, UserToken userToken) {
    if (csvContents == null) {
    ...
    try {
      ...
      String fileName = buildFileName(userToken);
      File file = new File(dir, fileName);
      FileUtils.writeStringToFile(file, csvContents);
      return Optional.of(file);
    } catch (IOException e) {
      logError("Error in file creation", e);
      return Optional.empty();
    }
  }

  private File createDirectoryIfNotExists() throws IOException {
  ...
  private String buildFileName(UserToken userToken) {
  ...
  private Message raiseTSUPTicket(File file) {
    try {
      ...
      logError("Error in raising TSUP ticket", e);

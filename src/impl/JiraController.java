...
public class JiraController {
    ...
    public Message raiseJiraTicket(@RequestBody MultipartFile data) throws IOException {
        ...
        try {
            ...
            file = handleFileOperations(csvContents, userToken);
    ...
    private void validateCSVContents(String csvContents) {
        if (csvContents == null) {
            throw new IllegalArgumentException("CSV contents cannot be null");
        }
    }

    private File handleFileOperations(String csvContents, UserToken userToken) throws IOException {
        File dir = createTemporaryDirectory();
        return writeCSVContentsToFile(csvContents, dir, userToken);
    }
    ...
    private File writeCSVContentsToFile(String csvContents, File dir, UserToken userToken) throws IOException {
        // doing null check 
        if (csvContents != null) {
            File file = new File(dir, generateFileName(userToken));
            writeFileContent(file, csvContents);
            return file;
        }
        return null;
    }

...
public class JiraController {
    ...
    private File createFileFromCSVContents(String csvContents, UserToken userToken) throws IOException {
        ...
        try {
            ...
            logErrorAndRethrow(e, "Error writing CSV contents to file");
            return null; // This line will never be reached due to the exception being rethrown
        }
    }
    ...
    private Message raiseJiraTicketWithFile(File file) throws IOException {
        try {
            return jiraService.raiseTSUP(file);
        } catch (IOException e) {
            logErrorAndRethrow(e, "Error in raising Jira ticket");
            return null; // This line will never be reached due to the exception being rethrown
        }
    }

    private void logErrorAndRethrow(IOException e, String errorMessage) throws IOException {
